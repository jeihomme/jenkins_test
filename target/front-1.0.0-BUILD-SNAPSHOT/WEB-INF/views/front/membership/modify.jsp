...
<div class="mypage_subGnb">
	<ul>
		...
		<li class="active"><a href="/front/membership/modify" title="정보수정">정보수정</a></li>
	...
	<div class="sectionWrap">
		<div class="membership_modify">
			...
			<div class="form_control modify_form">
				...
				<div class="modify_form_content join_form_left">
					<h4>이름</h4>
					<p><input id="modifyFormNick" type="text" name="modifyFormName" title="닉네임 입력" disabled / value="${data.USER_NAME}"></p>
					<h4>닉네임</h4>
					...
					<h4>비밀번호</h4>
					<p><input id="modifyFormPW" type="password" name="modifyFormPW" title="비밀번호 입력" placeholder="비밀번호(문자, 숫자 포함 8자 이상)을 입력해주세요."/></p>
					<h4>비밀번호 재입력</h4>
					<p><input id="modifyFormPWR" type="password" name="modifyFormPWR" title="비밀번호 재입력" placeholder="비밀번호를 재입력해주세요."/></p>
					<h4>직업</h4>
				...
				<div class="modify_form_content join_form_right">
					<h4>이미지 수정</h4>
					<p class="myInfo_pic">
...
//2. 유효성 검사 서비스로 이동
var result = ${unit.ajax("modify", "params")};
...
function fn_modifySubmit() {
	...
	if( $('[name=modifyFormPW]').val() != $('[name=modifyFormPWR]').val() ){
		alert("비밀번호가 일치하지 않습니다"); 
		$('[name=modifyFormPW]').val('');
		$('[name=modifyFormPWR]').val('');
		return;
		}
...
//	1. 유효성 검사
	...
	params.modifyFormPW = $('[name=modifyFormPW]').val();
	params.modifyFormPWR = $('[name=modifyFormPWR]').val();
