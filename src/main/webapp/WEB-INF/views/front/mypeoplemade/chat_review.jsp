...
<c:set var="ajaxUrl" value="${fn:replace(requestScope['javax.servlet.forward.servlet_path'], 'chat_review', 'ajax')}" />
...
.nowHist .sh_name{ font-weight: bold; }
...
$('.emotion_btn').click(function(){
	if( $('.emotion_list').css('display') == 'none' ) {
		...
		if( emotionCnt == 0 ) {
			...
			$.each( fileArr, function(key,val){
				inTag += '<img src="'+ fileArr[key] +'" style="width:20px;height:20px;" onclick="$(\'#chatText\').val( \''+ fileArr[key] +'\' );insertText( \'P\' );"/>';		
			});
...
// 						alert( value.USER_TYPE + ', ' + var_user_type  + ' : ' + value.USER_IDX.toString()  + ', ' + var_user_idx.toString() );
					...
					if( value.USER_UUID != null && value.USER_UUID != '' && value.USER_UUID != undefined ){
						$("[idx="+value.IDX+"] > span").css({"background-image":"url("+$('#imgTag').val()+")"});
					}else{
						$("[idx="+value.IDX+"] > span").css({"background-image":"url(/resources/assets/images/sub/profile_default_img.png)"});
					}
...
function fn_MastReviList(){
	...
	if ( unit_isComplete(result) ) {
		if( jsonData.length > 0 ) {
			$('#reviList').empty();
			console.log( var_chat_room ); 
			$.each(jsonData, function(key, val){

				console.log( val.BUY_IDX ); 
				setUuid( val.USER_UUID );
				unit_fileGet( val.USER_UUID_NM+'_FILE' , "fn_fileUpload");
...
function fn_history_forward( MOVE_URL ){
	location.href = MOVE_URL.replace('/prdt/detail','/mypeoplemade/chat_review');
	return;
}
