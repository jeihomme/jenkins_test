...
<div class="tbl_view">
	<table class="view_tbl01" summary="" id="searchArea">
		...
		<tbody>
			<tr>
				<th scope="row">제목</th>
				<td><input type="text" name="notiTitl" class="form-control form-control-sm col-8"  placeholder="아이디를 입력하세요" value="${out.params.notiTitl }"  onkeypress="if(event.keyCode==13){fn_BtnSearch();}"/></td>
				<th scope="row">등록자</th>
				<td><input type="text" name="sysRegIdx" class="form-control form-control-sm col-8"  placeholder="관리자명을 입력하세요" value="${out.params.sysRegIdx }"  onkeypress="if(event.keyCode==13){fn_BtnSearch();}"/></td>
			...
			<c:choose>
				<c:when test="${fn:length(out.list.data) > 0}">
					...
					<tr>
						...
						<td><c:if test="${not empty row.NOTI_UUID && row.NOTI_UUID ne ''}"><img style="height: 15px;" src="/resources/images/icon_file.png" alt="첨부파일 이미지" title="첨부파일 이미지"> </c:if><a href="${ unit.move('view', row.NOTI_IDX, 'mng') }">${ row.NOTI_TITL } </a></td>
