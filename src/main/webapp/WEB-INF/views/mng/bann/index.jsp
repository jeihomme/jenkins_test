	...
	<table class="view_tbl01" summary="" id="searchArea">
		...
		<tbody>
			<tr>
				...
				<th scope="row">제목</th>
				<td><input type="text" name="bannTitl1" class="form-control form-control-sm col-8"  placeholder="제목을 입력하세요" value="${out.params.bannTitl1 }"  onkeypress="if(event.keyCode==13){fn_BtnSearch();}"/></td>
...
<!--  INFO |---------|----------|----------|-----------|------------|------------|---------|--------|  -->
<!--  INFO |BANN_IDX |BANN_TYPE |BANN_ORDR |BANN_TITL1 |BANN_USE_YN |SYS_REG_DTM |NUM_DESC |NUM_ASC |  -->
<!--  INFO |---------|----------|----------|-----------|------------|------------|---------|--------|  -->
...
<%-- 						<td>${row.NUM_DESC_ORDER }</td> --%>
						<td>${row.NUM_DESC_ORDER }</td>
						<td><a href="${ unit.move('edit', row.BANN_IDX, 'bann') }">${ row.BANN_TYPE }</a></td>
						<td>${row.BANN_BANN_TITL}</td>
						<td>${row.BANN_ORDR }</td>
