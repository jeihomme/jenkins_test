	...
	<table class="list_tbl01" summary="">
		...
		<thead>
			<tr>
				...
				<th scope="col">메인노출<br>순서</th>
				<th scope="col">구분</th>
				<th scope="col">서브노출</th>
				
				<th scope="col">서브노출<br>순서</th>
			...
			<c:choose>
				<c:when test="${fn:length(out.list.data) > 0}">
					...
					<tr>
						...
						<td>
							<c:if test="${row.MAIN_OPEN_YN eq 'Y'}"><input type="text" style="width: 65%;" value="${row.MAIN_OPEN_ORDR }"/></c:if>
							<c:if test="${row.MAIN_OPEN_YN eq 'N'}">${row.MAIN_OPEN_ORDR }</c:if>
						</td>
						<td>${row.PRDT_TYPE }</td>
						<td>${row.SUBMAIN_OPEN_YN }</td>
						<td>
							<c:if test="${row.SUBMAIN_OPEN_YN eq 'Y'}"><input type="text" style="width: 65%;" value="${row.SUBMAIN_OPEN_ORDR }"/></c:if>
							<c:if test="${row.SUBMAIN_OPEN_YN eq 'N'}">${row.SUBMAIN_OPEN_ORDR }</c:if>
						</td>
						...
						<td>${row.S_CODE_NM }</td>
						<td>
							<a class="btn insertPopup" style="    background-color: #1eccb4 !important;    border-color: #1eccb4;" href="javascript:;" onclick="selectMngMdpickDetail('${row.PRDT_REG_CD }');">수정</a>
						</td>
					</tr>
					</c:forEach>
				</c:when>
...
<div class="btn_wrap" style="text-align: inherit;">
	...
	<a class="btn insertPopupOrdr" style="    background-color: #ccc !important;    border-color: #ccc;" href="javascript:;" >순서저장</a>
</div>
...
<div id="popup_insertPopup" class="popup_wrap">
	<div class="popup_layer">
		<div class="popup_inner">
			<div class="popup_header">	
				<h1 id="mdpickOpenPopup" class="title" style="margin:0; font-weight:600 !important;">MD's Pick 노출여부</h1>			
				<button onclick="fn_cleanPop();" class="btn btn_close_pop" data-action="popup_hide">닫기</button>
			</div>
			<div class="dialog_content">
				<div class="scroll_area">
					...
					<div class="tbl_view" >
						<table class="view_tbl01" summary="" id="subOpen">
							...
							<tbody>
								...
								<tr>
									<th scope="row">메인 Md Pick 여부</th>
									<td>
										<div class="designRadiobutton form-control-sm">
											<input type="radio" id="radio4_1" name="MAIN_OPEN_YN_DETAIL" value="Y" checked />
											<label for="radio$_1" class="label_txt">Y</label>
										</div>
										<div class="designRadiobutton form-control-sm">
								...
								<tr>
									<th scope="row">메인 Md Pick 노출순서</th>
									<td><input type="text" id="MAIN_OPEN_ORDR_DETAIL" class="form-control form-control-sm col-8" value="" /></td>
								</tr>
								<tr>
									<th scope="row">서브 Md Pick 여부</th>
									<td>
										<div class="designRadiobutton form-control-sm">
								...
								<tr>
									<th scope="row">서브 Md Pick 노출순서</th>
									<td><input type="text" id="SUBMAIN_OPEN_ORDR_DETAIL" class="form-control form-control-sm col-8" value="" /></td>
								</tr>
							</tbody>
						</table>
					</div>
...
//////////////////////////////////////////////////////////
	...
	function fn_BtnRegistMain(){
		$('input[name="chk"]:checked').each(function(key,val){
			...
			params.status = "WRITE";
			params.query = "insertMdpickMainOpenYnData";
	...
	function fn_BtnRegistSub(){
		$('input[name="chk"]:checked').each(function(key,val){
			...
			params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
			params.status = "WRITE";
			params.query = "insertMdpickMSubOpenYnData";
			params.prdtCd = $(this).val();
	...
	function fn_BtnRegistOrdr(){
		$('input[name="chk"]').each(function(key,val){
			if( $(this).is(":checked") ) {
				...
				params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
				params.status = "WRITE";
				params.query = "insertMdpickMOrdrData";
				params.prdtCd = $(this).val();
				params.MAIN_OPEN_ORDR = $('.list_tbl01 tbody tr').eq(key).find('td').eq(3).find('input').val();
				params.SUBMAIN_OPEN_ORDR = $('.list_tbl01 tbody tr').eq(key).find('td').eq(6).find('input').val();
	...
	function selectMngMdpickDetail( PRDT_REG_CD ){
		var params = new Object();
		params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
		params.status = "DETAIL";
		params.query = "selectMngMdpickDetail";
		...
		if( result.length > 0 ) {
			$.each(result, function(key,val){
				...
				$('#MAIN_OPEN_ORDR_DETAIL').val(val.MAIN_OPEN_ORDR);
				...
				$('#SUBMAIN_OPEN_ORDR_DETAIL').val(val.SUBMAIN_OPEN_ORDR);
	...
	function fn_BtnRegist(){
		...
		params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
		params.status = "WRITE";
		params.query = "insertMdpickData";
		params.prdtCd = $('#PRDT_REG_CD').text();
		params.MAIN_OPEN_ORDR = $('#MAIN_OPEN_ORDR_DETAIL').val();
		...
		if( $(this).is(":checked") ) $('[name=MAIN_OPEN_YN_DETAIL]:radio[value="Y"]').prop('checked',true);

		params.SUBMAIN_OPEN_ORDR = $('#SUBMAIN_OPEN_ORDR_DETAIL').val();
