	...
	<table class="view_tbl01" summary="" id="searchArea">
		...
		<tbody>
			<tr>
				...
				<td>
					<select id="faceStatCd" name="faceStatCd" class="form-control form-control-sm col-8">
			...
			<c:choose>
				<c:when test="${fn:length(out.list.data) > 0}">
					...
					<tr>
						...
						<td>${ row.MASTER_NM } </td>
						<td><a href="${ unit.move('edit', row.FACE_IDX, 'face') }">${ row.FACE_CD } </a></td>
<%-- 						<td><a href="${ unit.move('edit', row.FACE_IDX, 'face') }">${ row.FACE_NM } </a></td> --%>
<%-- 						<td>${row.FACE_STR_DTM }</td> --%>
						<td>${row.TIME_RUN}</td>
						<td>${row.FACE_PRIC}</td>
						<td>${row.FACE_DISC_CD }</td>
						<td>${row.FACE_DISC }</td>
						<td>${row.FACE_PRIC_REAL }</td>
						
						<td>${row.SYS_REG_DTM }</td>
						<td>${row.FACE_STAT_NM }</td>
						<td><a class="btn " href="${ unit.move('write', row.FACE_IDX, 'face') }">클래스복사</a></td>
					</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
...
//////////////////////////////////////////////////////////
$(function(){
	fn_SelectOption("","SAL","faceStatCd");
	$('#faceStatCd').val( '${out.params.faceStatCd }' );
