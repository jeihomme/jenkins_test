	...
	<table class="view_tbl01" summary="" id="searchArea">
		...
		<tbody>
			<tr>
				...
				<td>
					<select id="mailStatCd" name="mailStatCd" class="form-control form-control-sm col-8">
	...
	<table class="list_tbl01" summary="">
		...
		<thead>
			<tr>
				...
				<th scope="col">사용기한(일)</th>
				
				<th scope="col">이용가능 횟수(회)</th>
			...
			<c:choose>
				<c:when test="${fn:length(out.list.data) > 0}">
					...
					<tr>
						...
						<td>${ row.MASTER_NM } </td>
						<td><a href="${ unit.move('edit', row.MAIL_IDX, 'mail') }">${ row.MAIL_CD } </a></td>
						<td>${row.P_CODE_NM}</td>
						<td>${row.M_CODE_NM}</td>
						<td>${row.MAIL_DTM_CNT }</td>
						
						<td>${row.MAIL_CNT}</td>
						<td>${row.MAIL_PRIC}</td>
						<td>${row.MAIL_DISC_CD }</td>
						<td>${row.MAIL_DISC }</td>
						<td>${row.MAIL_PRIC_REAL }</td>
						
						<td>${row.SYS_REG_DTM }</td>
						<td>${row.MAIL_STAT_NM }</td>
						<td><a class="btn " href="${ unit.move('write', row.MAIL_IDX, 'mail') }">클래스복사</a></td>
					</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
...
//////////////////////////////////////////////////////////
$(function(){
	fn_SelectOption("","SAL","mailStatCd");
	$('#mailStatCd').val( '${out.params.mailStatCd }' );
