...
and (max-device-width : 1200px) {	
	.Chkbox input[type="checkbox"] + label.label_txt{ padding: 0 0 0 25px; }
	.id_menu ul ul > li{ margin-bottom: 12px; }
	.id_menu ul{ height: 350px; }
}
</style>
...
<div class="btn_wrap" style="position: relative; text-align: right;">
	...
	<c:if test="${out.params.prevIdx > 2}"><a class="btn  btn-md btn-danger" onclick="fn_delete();" >삭제</a></c:if>
...
$(function(){
	...
	params.query = "selectMngPrevDetail";
	params.PREV_IDX = '${out.params.prevIdx }';	
	params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
	
	var result = fn_DataDetail( params );
// 	result[0].PREV_CD;
	
	$('input:checkbox[name^=checkbox]').each(function(k){
		if ( result[0].PREV_CD.includes( this.value )	 ){
			if ( this.value != "" ) {
				this.checked = true;
			}
		} 
	...
	$('input:checkbox[name^=checkbox][value=""]').each(function(k){
		
		var legnth = $('[name=checkbox'+k+']').length;
		var legnthCk = $('[name=checkbox'+k+']:checked').length;
		if( (legnth-1) == legnthCk ) $("input[name=checkbox"+k+"]").prop("checked",true);
...
if ( $('[name=PREV_NM]').val().trim() == '' || $('[name=PREV_NM]').val().trim() == null ) return alert('권한 그룹명을 입력해 주세요.');
	...
	var params = new Object();
	params.status = "EDIT";
	params.query = "updateMngPrevData";
	params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
...
function fn_delete(){
	if ( confirm('정말로 삭제하시겠습니까?') ) {
		...
		params.query = "deleteMngPrevData";
		params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
		params.PREV_IDX = '${out.params.prevIdx }';	
		fn_DataAjax( params );
		location.href='${unit.move("index", "" )}';
