	...
	<table class="view_tbl01" summary="" id="searchArea">
		...
		<tbody>
			...
			<tr>
				...
				<td colspan="3" style="text-align:left;">
					<div class="designRadiobutton form-control-sm">
						<input type="radio" id="radio0_Y" name="LIVE_STAT_CD" value="SAL01" checked/>
						<label for="radio0_Y" class="label_txt">판매대기</label>
					</div>
					<div class="designRadiobutton form-control-sm">
						<input type="radio" id="radio0_N" name="LIVE_STAT_CD" value="SAL02" />
						<label for="radio0_N" class="label_txt">판매중</label>
					</div>
					<div class="designRadiobutton form-control-sm">
						<input type="radio" id="radio1_N" name="LIVE_STAT_CD" value="SAL03" />
							...
							<table class="view_tbl01" summary="" id="searchAreaTime">
								...
								<tbody>
									...
									<tr>
										...
										<td style="text-align:left;">
											<div id="TIME_DTM_DIV" style="width:100%;display: inline-block;"></div>
											<textArea id="TIME_DTM" name="TIME_DTM" class="form-control form-control-sm col-8" type="text" style="display:none;" disabled></textArea>
										</td>
									</tr>
									<tr>
										<th scope="row">시작</th>
...
// 	fn_SelectOption("","PLA","LIVE_PLAY"); 
	...
	if( params.liveIdx != '' ) {
		...
		$('[name=LIVE_STAT_CD]:radio[value="'+jsonData.out.detail.data[0].LIVE_STAT_CD+'"]').prop('checked',true);
	 	...
	 	var codeIdStr = $('[name=MAIN_CODE_ID]').val()+'';
...
function fn_BtnRegist(){
	if ( confirm('저장 후에는 마스터 선택란을 변경할 수 없습니다') ) {
		...
		params.status = "WRITE";
		params.query = "insertMngLiveData";
		params.mngLoginIdx = localStorage.getItem("mngLoginIdx");
		params.LIVE_CD = '${sysDateValue}'+'LN';
