	...
	<table class="view_tbl01" summary="" id="searchArea">
		...
		<tbody>
			<tr>
				...
				<td>
					<select id="liveStatCd" name="liveStatCd" class="form-control form-control-sm col-8">
			...
			<c:choose>
				<c:when test="${fn:length(out.list.data) > 0}">
					...
					<tr>
						...
						<td>${ row.MASTER_NM } </td>
						<td><a href="${ unit.move('edit', row.LIVE_IDX, 'live') }">${ row.LIVE_CD } </a></td>
						<td>${row.TIME_RUN}</td>
						<td>${row.LIVE_PRIC}</td>
						
						<td>${row.LIVE_DISC_CD }</td>
						<td>${row.LIVE_DISC }</td>
						<td>${row.LIVE_PRIC_REAL }</td>
						<td>${row.SYS_REG_DTM }</td>
						<td>${row.LIVE_STAT_NM }</td>
						
						<td><a class="btn" href="${ unit.move('write', row.LIVE_IDX, 'live') }">클래스복사</a></td>
					</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
...
//////////////////////////////////////////////////////////
$(function(){
	fn_SelectOption("","SAL","liveStatCd");
	$('#liveStatCd').val( '${out.params.liveStatCd }' );
