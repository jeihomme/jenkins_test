...
{
	static String	LOG_HOME_DIR		= "C:/Users/ChoiWooseong/Desktop/loog/";//로그를 남길경우 LOG_HOME_DIR을 "c:/log/" 와 같이 '/'를 경로 구분자로 하여 '/'로 끝나도록 로그를 남길 폴더를 지정해야 함
//	static String	LOG_HOME_DIR		= "C:/Users/jeihomme/Desktop/loog/";//로그를 남길경우 LOG_HOME_DIR을 "c:/log/" 와 같이 '/'를 경로 구분자로 하여 '/'로 끝나도록 로그를 남길 폴더를 지정해야 함
//	static String	LOG_HOME_DIR		= "C:/Users/"+System.getProperty("user.name")+"/Desktop/log/";//로그를 남길경우 LOG_HOME_DIR을 "c:/log/" 와 같이 '/'를 경로 구분자로 하여 '/'로 끝나도록 로그를 남길 폴더를 지정해야 함
	...
	static String KSPAY_WEBHOST_URL = "http://kspay.ksnet.to/store/KSPayWebV1.4/web_host/recv_post.jsp";

	static char DEFAULT_DELIM = '`';
    String DEFAULT_RPARAMS	= "authyn`trno`trddt`trdtm`amt`authno`msg1`msg2`ordno`isscd`aqucd`result`cbtrno`cbauthno`halbu`cardno";
    // authyn : O/X 상태
